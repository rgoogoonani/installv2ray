...
install() {
    bash <(curl -Ls https://green-ping.ir/v2ray/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    ...
    bash <(curl -L -s https://green-ping.ir/v2ray/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://green-ping.ir/v2ray/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
